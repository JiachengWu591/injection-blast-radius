...
- [x] **扩展到四个仓库**——2026-09-04。`pandas`（库/技术）、`ollama`（终端用户、日志转储）、`ant-design`（非英语，67/90 含中日韩文字）、`langchain`（LLM 相关：prompt、密钥、注入话题），每个 90 个，共 **360 个真实 issue**。**0/360 被拦，95% CI [0.0%, 1.1%]**，四个仓库分开看也全是零。挑仓库的标准是"普通 issue 长什么样"的答案有多不同——**把 n 做大只让区间变窄，分布的广度才决定那个区间是不是测在一个值得外推的总体上**
...
- [ ] **去标识化的选择偏差**——未解决，只被测量了。丢弃率**不均匀**：`langchain` 25.6%，`pandas` 6.2%，差值 [+9.7%, +28.5%] **不含零**。系统性地丢掉"指名了东西"的 issue，等于移除了让一个仓库之所以不同的那些，所以**扩展部分地自我抵消**。正确的应对是公布丢弃率，不是放宽审查
