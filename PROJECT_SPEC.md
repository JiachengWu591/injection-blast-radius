...
- [ ] **接入真实数据**——刻意未做。第 6 节第 172/173 行禁止真实第三方 issue（真实人名/产品名属于个人信息，且"issue 内容全部用本地文件模拟"），所以语料是合成的。**这也是那个 0/165 的约束性局限**：写语料的是模型、审计的也是模型，相关盲区无法排除。要推动它需要一个独立的、像第 175 行对待真实 API 那样的治理决定
