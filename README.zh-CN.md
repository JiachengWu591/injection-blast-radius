...
| **白名单。** `suggested_action` 先对照一个固定元组校验，再经由一个 `match` 分派，其分支就是这个系统能做的全部事情。 | [`ibr/executor.py:122`](ibr/executor.py#L122)（枚举校验）和 [`ibr/executor.py:128-172`](ibr/executor.py#L128-L172)（那个 `match`） |
