...
已发表的提示词层面防御在自适应攻击者面前表现不佳——有报告称对它们的绕过率远超 90%。这个数字是**转述的、没有引用**的，在一份其他每个数字都带区间、都能被脚本复现的文档里，这一点值得点出来：它来自一般文献，不来自本仓库，而**本项目从未对自己的审计跑过一次自适应攻击**（见[诚实的局限](#诚实的局限)）。把它当作架构为什么长这样的**理由**，不要当作**证据**。这是预期结果，不是失败——而这个结论不依赖那个量级：**一道用与被防御对象相同材质造出来的防线，终究会被足够好的攻击击穿。**
...
有两件事这个测量**不是**。它是**每个 issue 跑一次**、不是三次，所以 Reader 自身的非确定性意味着**重跑一遍会挑出不同的十个**；这个比率估计的是"这件事多久发生一次"，不是"它发生在哪几个 issue 上"。以及，它和本页其他一切都测在**同一份合成语料**上——见[这次测量不能告诉你什么](#这次测量仍然不能告诉你什么)。
...
包是怎么分层的、每个数据契约装什么、边界在哪里，见 [ARCHITECTURE.zh-CN.md](./ARCHITECTURE.zh-CN.md)——两张 UML 图都对着 `dataclasses.fields()` 校验过，所以它们不可能和代码脱节。fail-closed 规则可以直接当断言来读：[`tests/test_failure_paths.py`](tests/test_failure_paths.py)，每一条都写着它那个响应为什么只能构造、不能录制。

[PROJECT_SPEC.md](./PROJECT_SPEC.md) 是所有者的实现规格与治理记录——各 Phase 的验收标准、硬性安全约束、以及授权拉取真实 issue 文本的那条记录在案的例外。它是**这个项目被允许做什么**的事实来源。
...
| **白名单。** `suggested_action` 先对照一个固定元组校验，再经由一个 `match` 分派，其分支就是这个系统能做的全部事情。 | [`ibr/executor.py:195`](ibr/executor.py#L195)（枚举校验）和 [`ibr/executor.py:201-262`](ibr/executor.py#L201-L262)（那个 `match`） |
| **静态输出集。** 系统能发布的**评论正文**的每一个字节，全部枚举完。没有任何模型生成的内容被插值进去；而唯一会进入已发布行的、来自 issue 的值——sink 自己那圈框架里的 id——在 frozen dataclass 上被约束成 `[A-Za-z0-9._-]{1,64}`，发布前还会被输出审计再独立扫一遍（仅限已知密钥形状）。 | [`ibr/executor.py:62`](ibr/executor.py#L62) |
| **跨越点。** 这行以上的代码见过原始不可信文本。这行以下，**能到达 Executor** 的只有那个已校验对象的两个枚举字段——自由文本还会再被读一次写进日志记录，然后就到此为止。 | [`ibr/pipeline.py:509`](ibr/pipeline.py#L509) |
...
而关键在于：`required_samples_per_group` 算出来，要在 80% 功效下分辨 4.0% 与 0.09% 的差距，需要约 **202 样本/组**。之前 n=25 的那几次运行不只是噪声大，而是**离能看到数据中唯一真实效应差了整整一个数量级**。我自己在这里先后写错过两个结论——先是"总是同一个载荷漏"，后是"没有载荷稳定地漏"——**两次都是靠加样本量纠正的，不是靠想得更明白。**
...
| `ant-design`——非英语为主，66/90 含中日韩文字 | 90 | 0 | [0.0%, 4.1%] | 1 |
| `langchain`——LLM 相关：prompt、API key、注入话题 | 90 | 0 | [0.0%, 4.1%] | 4 |
| **真实数据合计** | **360** | **0** | **[0.0%, 1.1%]** | **7** |

**0/360，95% CI [0.0%, 1.1%]**——而且**四个仓库分开看也全是零**，尽管这些 issue 在形态上几乎没有共同点。正文长度中位数从 734 字符（`ant-design`）到 3,636（`langchain`）；其中一个仓库三分之二是中文；**230 个不同作者**。

1,080 次调用里有 7 次判 `suspicious`。**有一次判了 `high_risk`**——一个再普通不过的 `langchain` bug 报告，在它三次调用中的一次上，另外两次都判 safe。那**一票**正是逐 issue 判定要取多数的全部理由：换成单次调用判定，这个 issue 就被拦了，真实数据上的比率就是 1/360 而不是 0/360。报告里"三次里至少一次被拦"那一行就是这个悲观读法——1/360，[0.0%, 1.6%]。

这一节早先的版本把合成语料称作**更难的那一份**，依据是 `suspicious` 率 2/165 = 1.2% 对当时测到的 4/360 = 1.1%。**零点一个百分点撑不起这个结论**；而重新拉取后的语料把真实数据的率放到 7/360 = 1.9% [0.9%, 4.0%] 对 1.2% [0.3%, 4.3%]——差值 [−2.5%, +2.9%]。**符号翻了，两个区间却都没怎么动**，这正是"没有差别"长的样子。

**站得住的是那个更弱的说法**，而这一节需要的恰好就是它：合成语料对审计器**并没有系统性地更容易**，所以那个干净的结果不是"模型给模型写的东西打分"造出来的人造产物。两份语料做的仍然是两件不同的事——合成的那份是**造在误报边界上的压力测试**，真实的那些是**检查那个压力测试测到了东西**。
...
| `pandas` | 91 | 1 | 1.1% | [0.2%, 6.0%] | 基准 |
| `ollama` | 93 | 3 | 3.2% | [1.1%, 9.1%] | [−3.2%, +8.0%] |
| **`langchain`** | 107 | **17** | **15.9%** | [10.2%, 24.0%] | **[+7.3%, +22.9%]** |

`langchain` 与 `pandas` 的差值**仍然不含零**。而**按原因分组**能说出这个差值是由什么构成的——其中大部分**不是**去标识化：

| 原因 | `ant-design` | `pandas` | `ollama` | `langchain` |
|---|---|---|---|---|
| 能认出某个自然人 | · | · | 1 | 8 |
| 把报告者和某个组织绑在一起 | · | · | · | 4 |
| 是广告，不是 issue | · | · | · | 13 |
| 不是一个普通 issue | 1 | 1 | 2 | 11 |
| **丢弃的 issue 数** | **1** | **1** | **3** | **17** |

计数是**按原因**算的，所以一列加起来会超过它的丢弃数：一条既是广告又指名了作者的记录，是一次丢弃、落在两个表头下。`langchain` 那 17 条里有**十三条是厂商广告**——要求把某个第三方包列进文档的投稿。**那是那个 issue tracker 的属性，不是去标识化器的属性。**

这次拆分本该在**留下来的那些**里也看得见，而这一节早先的版本正是在这里发布了一个它没有挣到的数字。它写的是**保留的 360 条里有 156 条指名了第三方**，依据是每条记录上的 `names_third_party` 字段，并且据此论证：有了这个字段，"凡是提到组织的都被丢了"和"那些被留下了"事后是**可区分的**，而不是一个只能被相信的说法。

**那个字段测的不是这件事。** 它是 `"organisation" in reasoning.lower()`——在审查的自由文本 reasoning 上做一次**单一拼写**的子串匹配，而没有任何地方要求模型用这个词。在这份语料里，同一次调用的 `note` 字段出现 "organiz" 33 次、"organis" 31 次；被这个字段判为 **false** 的 204 条里，有 31 条自己的 note 里写着 "organiz"。和它 AND 在一起的那个布尔值是**死代码**：那一行只对已经通过了组织那一问的记录执行，所以恒为真。而结果本身早就自带反证——这个计数把 `pandas` 排在 `langchain` 前面，而 `langchain` 恰恰是那个 issue 频繁指名第三方包、以至于 17 条丢弃里13 条是厂商广告的 tracker。

现在 `names_third_party` 是审查的**第五个问题**，和另外四个一样是**判断出来的**，并且有一条断言钉住：不许再从 `reasoning` 里推导它。计数等下一次拉取给出。在那之前这一段**不带数字**——对一个"测量方式后来发现是拼写检查"的说法来说，这才是正确的证据量。

把两类原因分开，就能给"真正属于隐私过滤"的那部分**定出上下界**：

- **382 条里至少 5 条**（1.3%，[0.6%, 3.0%]）是**只因为隐私原因**被丢的；
- **最多 11 条**（2.9%，[1.6%, 5.1%]）**带有任何**隐私原因，把那些同时也是广告的算进来。

按下界算，`langchain` 与 `pandas` 的差值是 [−0.9%, +9.2%]，**含零**；按上界算是 [+3.5%, +16.4%]，不含零。你信哪个，取决于**一条有多个原因的丢弃该归给谁**，而这 382 个 issue 定不了这件事。所以**偏差收窄了，但没有消失**——活下来的 `langchain` 样本仍然偏向它那些**最像普通 bug 报告**的 issue。

**早先那个数字去哪了。** 这个拉取的上一轮丢掉了 `langchain` 的 25.6%，对比 `pandas` 的 6.2%。**去读那些被丢的 issue** 正是上面这张分组表的来由：当时一个 `identifies_someone` 布尔值同时干着三件无关的事，而且在**因为提到了自己调用的 API 就丢掉真正的 bug 报告**——其中一条丢弃理由自己写的是"一份维护者会正常处理的直白安全 bug 报告"。现在审查问的是**四个分开的问题**，而 [`tests/test_fetch_real.py`](tests/test_fetch_real.py) 有断言钉住它们**保持分开**，因为把它们再合回去，会悄悄抹掉让每个仓库之所以不同的东西。

对剩下的那部分，正确的应对仍然是**把丢弃率公布出来**，不是放宽审查——[§6.1](./PROJECT_SPEC.md) 不允许后者。
...
**零结果依然是零结果。** 360 个 issue 里**零个被拦**，把这个率的**上界**限定在 1.1%。1,080 次调用里**确实有一次**返回了 `high_risk`，所以这里测到的**不是**一个等于零的比率——是一个小到 360 个 issue 分辨不出来的比率。

所以这个声称是**故意窄的**：这是审计对**这几种形状**的普通 issue 会怎么做——七个刻意造在误报边界上的合成分层，加四个真实 issue tracker。它对一个跟这些都不像的工单堆会怎么做，**没有**被确立，而上面那条"四个仓库"的限制就是这件事的诚实说法。[`tests/test_corpus.py`](tests/test_corpus.py) 里那 **17** 条断言钉住了两份语料**各是什么**——每个密钥样字符串都带 fake 标记、诱饵密钥不存在、每个 author 都是 handle 形状、只有一个虚构产品家族——但**没有任何断言能让一个合成 issue 变得有代表性**。
...
- **误报率是在 165 个合成 issue 和 360 个真实 issue 上测的，而这两个 n 都分辨不出一个这么小的比率。** 见[上文](#这次测量仍然不能告诉你什么)。这一条原先写的是"误报率是在合成语料上测的"，并且提出共享盲区的担忧——生成器和审计器都是模型。四个真实仓库正是为了回答这个担忧才加的，而这一条没有跟着改，于是那个持怀疑态度的读者最信任的一节，一直在否认为了说服他们而做的工作。站得住的说法更窄、也确实成立：上界是 1.1%，1,080 次真实调用里**有一次**返回了 `high_risk`，而四个仓库全是 GitHub 上面向开发者的开源项目。
- **真实语料是经过一道不均匀的过滤活下来的，而这个偏差被测量了、没被解决。**去标识化在审过的 382 条里丢掉 22 条（5.8%），但并不均匀：`langchain` 15.9% 对 `pandas` 1.1%，差值 [+7.3%, +22.9%] **不含零**。这个差距的大部分是**厂商广告**而不是隐私——`langchain` 那 17 条里 13 条是——把两类分开，能给**真正属于隐私过滤**的那部分定出界：下界 5/382 [0.6%, 3.0%]，上界 11/382 [1.6%, 5.1%]。按下界算，`langchain` 与 `pandas` 的差**含零**；按上界算不含。你信哪个，取决于一条有多个原因的丢弃该归给谁，而这 382 个 issue 定不了。所以活下来的 `langchain` 样本仍然偏向它那些**最像普通 bug 报告**的 issue，而正确的应对仍然是**公布丢弃率**，不是放宽 [§6.1](./PROJECT_SPEC.md) 不允许放宽的那道审查。[细节](#那道过滤让这份语料的代表性比看起来更差)。
...
| `mise run corpus:real` | 拉取并去标识化 360 个真实 issue，四个仓库各 90（约 $0.20） | 是 |
