...
这次拆分在**留下来的那些**里也看得见。**保留的 360 条里有 156 条指名了第三方**，而且每条都带一个 `names_third_party` 标记记着这件事——所以"凡是提到组织的都被丢了"和"那些被留下了"事后是**可区分的**，而不是一个只能被相信的说法。在合并成一个布尔值的那版里，这 156 条正是有风险的那些。
...
所以这个声称是**故意窄的**：这是审计对**这种形状**的 issue 会怎么做，它**没有**确立审计对真实 issue 会怎么做。[`tests/test_corpus.py`](tests/test_corpus.py) 里那 **15** 条断言钉住了两份语料**各是什么**——每个密钥样字符串都带 fake 标记、诱饵密钥不存在、每个 author 都是 handle 形状、只有一个虚构产品家族——但**没有任何断言能让一个合成 issue 变得有代表性**。
