...
那份语料是合成的，所以最明显的反驳是：**写它的是模型、审计它的也是模型**，相关盲区恰好就会产生这么干净的结果。所以同一套测量在 **360 个真实 issue、四个仓库各 90 个**上又跑了一遍——只读拉取、去标识化后（`mise run corpus:real`，然后 `mise run fp-rate:real`）。

四个仓库而不是一个，挑选标准是它们对"**普通 issue 长什么样**"给出的答案有多不同——因为**把 n 做大只是让区间变窄，而分布的广度才决定那个区间是不是测在一个值得外推的总体上**。

| 语料 | n | 被拦 | 95% CI | `suspicious` |
|---|---|---|---|---|
| 合成、分层 | 165 | 0 | [0.0%, 2.3%] | 2 |
| `pandas`——库，技术密度高，贡献者有经验 | 90 | 0 | [0.0%, 4.1%] | 0 |
| `ollama`——终端用户为主：日志转储、Windows 路径 | 90 | 0 | [0.0%, 4.1%] | 2 |
| `ant-design`——非英语为主，67/90 含中日韩文字 | 90 | 0 | [0.0%, 4.1%] | 1 |
| `langchain`——LLM 相关：prompt、API key、注入话题 | 90 | 0 | [0.0%, 4.1%] | 1 |
| **真实数据合计** | **360** | **0** | **[0.0%, 1.1%]** | **4** |

**0/360，95% CI [0.0%, 1.1%]**——而且**四个仓库分开看也全是零**，尽管这些 issue 在形态上几乎没有共同点。正文长度中位数从 732 字符（`ant-design`）到 3,777（`langchain`）；其中一个仓库三分之二是中文；**232 个不同作者**。1,080 次调用里有 4 次判 `suspicious`，**没有一次判 `high_risk`**。

真正让我意外的是**方向**。**合成语料比任何一个真实仓库都更难**——它在 165 个 issue 上产出的 `suspicious` 比 360 个真实 issue 还多。原因是它的分层是**刻意造在误报边界上**的：真实 bug 报告通常不会贴出密钥样字符串、也不会把提示词注入当议题讨论。所以这两份语料做的是**两件不同的事**——合成的那份是**压力测试**，真实的那些是**检查那个压力测试测的不是一个人造产物**。

### 那道过滤让这份语料的代表性比看起来更差

去标识化会**丢弃任何散文里可能认出某人**的 issue，而丢弃率**并不均匀**：

| 仓库 | 审过 | 丢弃 | 比率 | 95% CI | 对比 `pandas` |
|---|---|---|---|---|---|
| `ant-design` | 94 | 4 | 4.3% | [1.7%, 10.4%] | [−9.2%, +5.0%] |
| `pandas` | 96 | 6 | 6.2% | [2.9%, 13.0%] | 基准 |
| `ollama` | 102 | 12 | 11.8% | [6.9%, 19.4%] | [−2.8%, +13.9%] |
| **`langchain`** | 121 | **31** | **25.6%** | [18.7%, 34.1%] | **[+9.7%, +28.5%]** |

`langchain` 与 `pandas` 的差值**不含零**，所以这是**真实效应而不是噪声**。它的 issue 就是在讨论组织、密钥和 prompt，而那恰好是审查器要丢的东西。

**这一点很要紧，而且它反过来削弱了这次扩展。** 系统性地丢掉"指名了东西"的 issue，等于系统性地移除了**让一个仓库之所以不同**的那些——所以活下来的 `langchain` 样本，偏向的是它那些**最像普通 bug 报告**的 issue。**扩展部分地自我抵消了**，而正确的应对是**把丢弃率公布出来**，不是放宽审查——[§6.1](./PROJECT_SPEC.md) 不允许后者。

### 这次测量**仍然不能**告诉你什么

**四个仓库是"普通 issue"的四个样本，不是这个概念的空间。** 四个都是 GitHub 上面向开发者的开源项目。一个商业客服工单系统、一个公司内部 tracker、或者一个用户会贴截图和发票的论坛，跟它们中任何一个都不像。

**合成语料仍然是合成的。** 真实数据这几轮排除的是相关盲区担忧里**最强的那个版本**——"生成出来的 issue 对审计器系统性地更容易"——**在 360 个 issue 所支持的分辨率上**，仅此而已。

**零结果依然是零结果。** 1,080 次调用里零次拦截，把这个率的**上界**限定在 1.1%；它**没有**确立这个率等于零。
...
| `mise run corpus:real` | 拉取并去标识化 165 个真实 pandas issue（约 $0.06） | 是 |
| `mise run fp-rate:real` | 同一套误报测量，跑在真实 issue 上 | 是 |
