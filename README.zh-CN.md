...
那份语料是合成的，所以最明显的反驳是：**写它的是模型、审计它的也是模型**，相关盲区恰好就会产生这么干净的结果。所以同一套测量在 **165 个来自 `pandas-dev/pandas` 的真实 issue** 上又跑了一遍——只读拉取、去标识化后（`mise run corpus:real`，然后 `mise run fp-rate:real`）。

| 语料 | 被拦 | 95% CI | 判 `suspicious` | 自己和自己不一致 |
|---|---|---|---|---|
| 合成、分层 | 0/165 | [0.0%, 2.3%] | 2/165 | 2/165 |
| **真实，`pandas-dev/pandas`** | **0/165** | **[0.0%, 2.3%]** | **0/165** | **0/165** |
| 差值（真实 − 合成） | —— | [−2.3%, +2.3%] | [−4.3%, +1.2%] | —— |

两个差值区间**都跨过零**，所以**没有检测到差异**。但要说清这是什么、不是什么：每组 165 个 issue 的情况下，这个实验需要**每组约 647 个**才能把 0.0% 和 1.2% 分开。这里的"没检测到差异"意思是**实验的功效不足以找到差异**，不是"合成语料已被确立为有代表性"。

真正让我意外的是**方向**。**合成语料比真实语料更难**——它产出了两个 `suspicious` 判决和两次 run-to-run 分歧，而真实的 pandas issue 两者都是零。原因是它的分层是**刻意造在误报边界上**的：真实 bug 报告通常不会贴出密钥样字符串、也不会把提示词注入当议题讨论。所以这两份语料做的是**两件不同的事**——合成的那份是**压力测试**，真实的那份是**检查那个压力测试测的不是一个人造产物**。

### 这次测量**不能**告诉你什么

**这里的"真实"指的是真实的 *pandas*。** 它的 issue 技术密度高、以英文为主、来自一个有经验的贡献者群体。一个面向普通消费者产品的支持工单系统长得完全不一样，而这里没有任何东西能说明审计在那种数据上会怎么做。**一个仓库只是"普通 issue"这个概念的一个样本。**

**而合成语料仍然是合成的。** 真实数据这一轮排除的是相关盲区担忧里**最强的那个版本**——"生成出来的 issue 对审计器系统性地更容易"——而且**只在这个 n 所支持的分辨率上**排除了它。
