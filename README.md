...
| **The crossing point.** Above this line, code has seen raw untrusted text. Below it, only the validated object's two enum fields reach the Executor — the free text is read once more into the log record and goes no further. | [`ibr/pipeline.py:510`](ibr/pipeline.py#L510) |
