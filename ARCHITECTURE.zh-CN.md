...
一个被完全攻陷的 Reader，能挑的是四个动作之一、四个 issue 类型之一。**十六种组合，在攻击者到场之前就已经枚举完了。** 而它唯一能随便写点什么进去的那两个字段，只到达日志，别处都到不了——[`ibr/executor.py:193`](ibr/executor.py#L193) 是那个 `match`，而整个文件除了日志记录之外**从不提及** `reasoning` 或 `summary`。
