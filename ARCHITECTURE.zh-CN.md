...
**下面没有一根边是编出来的**：每个箭头都是 [`tests/test_architecture.py`](tests/test_architecture.py) 真实找到的 import。但它**不是**完整的图——52 条真实边里画了 42 条。**省掉的十条全部指向 `config`**，有十二个模块从它那里读路径和常量；十二条全画出来会把整张图变成一把扇子、把结构埋掉。`llm` 和 `sandbox_fs` 的那两条保留为虚线，好让这个依赖至少在某处可见。
...
一个被完全攻陷的 Reader，能挑的是四个动作之一、四个 issue 类型之一。**十六种组合，在攻击者到场之前就已经枚举完了。** 而它唯一能随便写点什么进去的那两个字段，只到达日志，别处都到不了——[`ibr/executor.py:201`](ibr/executor.py#L201) 是那个 `match`，而整个文件除了日志记录之外**从不提及** `reasoning` 或 `summary`。
...
- **幂等是可选的，而且默认关闭。** 未包装的 sink 重跑一次就会发两次——因为 [`DEFAULT_SINK`](ibr/sinks.py) 写的是 `sandbox/`，在那里发重了不花任何代价。但你**不需要**自己造去重：`IdempotentSink(inner=YourSink())` 是一个两阶段账本——调用前落 intent、调用后落 `done`——遇到悬空 intent 时**拒绝执行**，而不是替你在"可能重复发布"和"可能丢动作"之间选一个。包不包是接入方的决定，而且 `run_batch` 还需要传 `already_done=` 才会跳过上一轮已完成的部分——所以 `batch` 和流水线本身都不是幂等的。
  ...
  在把真实 sink 包起来之前有一件事要知道：`run_batch` 默认开八个线程，而**账本的追加没有加锁**。对着 `sandbox/` 这没问题——追加是行级的，而最坏情况是一行被撕开，而现在的读取逻辑遇到它是**拒绝**而不是猜。但对着真实目标就不能这么假设：两个线程竞争的账本可能丢掉一条 `done`，而丢 `done` 恰恰是会让你重复发布的那种情况。要么把写串行化，要么给每个 worker 一个自己的分片。**这里没有任何东西替你做这件事，而且本仓库没有任何测试能抓到它**——三个线程写互不相同的 key，不是压力测试。
